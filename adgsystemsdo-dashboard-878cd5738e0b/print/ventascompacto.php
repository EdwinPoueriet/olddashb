...
//
//        function companyTotal ($company,$bruto,$desc,$itbs,$neto) {
...
//        }
        ...
        function printFooter($bruto,$desc,$itbis,$neto, $trasacciones) {
            echo ' <tfoot>
            <tr class=\'foooter\'>
            <td>Totales</td>
            <td>'.$trasacciones.'</td>
        ...
        if ($_POST['company'] == 'consolidado') {
            if ($_POST['consolidado_group'] == 'consolidado') {
                printHeader();
                foreach (json_decode($general->VentasReportCompact($_POST)) as $seller_id => $orders){
                    ...
                    foreach ($orders as $order) {
                     ...
                     <tr>
                        ...
                        <td>'.$order->seller_code.' - '.$order->seller_name.'</td>
                             <td >'.$order->COUNT.'</td>
                         <td style=\'text-align:right\'>'.convertNumber($order->bruto).'</td>
                      <td style=\'text-align:right\'>'.convertNumber($order->discount).'</td>
                         <td style=\'text-align:right\'>'.convertNumber($order->tax).'</td>
...
//                        printOrder($order);
                    ...
                    $grand_itbis += $itbis;
                    $grand_neto += $neto;
                    $grand_trasacciones += $trac;


                }
                echo "</tbody>";
                printFooter($grand_bruto,$grand_desc,$grand_itbis,$grand_neto, $grand_trasacciones);
                echo "</table>";
            }
            else if ($_POST['consolidado_group'] == 'separado') {
//                printHeader();
                foreach (json_decode($general->VentasReportCompact($_POST)) as $company => $sellers){
                    ///Company ?>
